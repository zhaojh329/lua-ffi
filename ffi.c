...
{
    switch (size) {
    case 1: { uint8_t v;  memcpy(&v, ptr, 1); return v; }
    case 2: { uint16_t v; memcpy(&v, ptr, 2); return v; }
...
static bool ctype_has_bitfield(struct ctype *ct)
{
    ...
    case CTYPE_RECORD:
        ...
        for (i = 0; i < ct->rc->nfield; i++)
            if (ctype_has_bitfield(ct->rc->fields[i]->ct))
                return true;
...
again:
        ...
        if (nfield >= MAX_RECORD_FIELDS)
            return luaL_error(L, "%d:too many fields (max %d)", yyget_lineno(), MAX_RECORD_FIELDS);
